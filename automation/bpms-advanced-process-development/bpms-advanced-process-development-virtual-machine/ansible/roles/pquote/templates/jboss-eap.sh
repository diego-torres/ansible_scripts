...
#
# chkconfig: - 80 20
# description: JBoss EAP BPMS startup script
# processname: bpms
# pidfile: /var/run/bpms/bpms.pid
# config: /etc/bpms/bpms.conf
#
...
if [ -z "$JBOSS_HOME" ]; then
	JBOSS_HOME={{lab_home_dir}}/bpms
fi
export JBOSS_HOME

if [ -z "$JBOSS_PIDFILE" ]; then
	JBOSS_PIDFILE=/var/run/bpms/bpms.pid
fi
export JBOSS_PIDFILE

if [ -z "$JBOSS_CONSOLE_LOG" ]; then
	JBOSS_CONSOLE_LOG=/var/log/bpms/console.log
fi
...
if [ -z "$JBOSS_LOCKFILE" ]; then
	JBOSS_LOCKFILE=/var/lock/subsys/bpms
fi
